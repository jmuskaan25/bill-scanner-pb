// ============================================
// Pronto Travel - Submissions Page Logic
// Django + JWT version
// ============================================
...
async function loadRecentSubmissions() {
  ...
  try {
    var resp = await fetch('/api/my-reimbursements', { headers: authHeaders() });
    ...
    if (docs.length === 0) {
      submissionsBody.innerHTML =
        ...
        '<p>No submissions yet. Upload a cab receipt to get started!</p></div></td></tr>';
      return;
    }
    ...
    docs.forEach(function(d) {
      ...
      var submitted = '--';
      if (d.created) {
        submitted = new Date(d.created).toLocaleDateString('en-IN', { day:'numeric', month:'short', year:'numeric' });
      }
      ...
      tr.innerHTML =
        '<td style="white-space:nowrap;">' + escapeHtml(submitted) + '</td>' +
